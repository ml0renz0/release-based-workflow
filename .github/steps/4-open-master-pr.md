...
## Paso 4: Abrir un *pull request* a la rama *master*

_¡Buen trabajo añadiendo la nueva funcionalidad!_ :smile:

### Rama *develop* y `master`

Abre una *pull request* entre tu rama `develop` y `master` lo antes posible.  

En general, la descripción de la PR debería incluir:

- Una [referencia a la *issue*](https://docs.github.com/es/articles/basic-writing-and-formatting-syntax/#mencionar-personas-y-equipos) que resuelve la PR.  
...
### :keyboard: Actividad: abrir una PR a master

Vamos a crear una PR comparando la rama `develop` con `master`.

1. Abre una **nuevo pull request** con `base: master` y `compare: develop`.  
